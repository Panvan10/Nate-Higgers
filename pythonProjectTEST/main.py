...
class TestCalendarAPI(unittest.TestCase):
    ...
    def test6_get_events_by_date_added_in_previous_test(self):
        global token
        self.headers['Authorization'] = f'Rearer {token}'
        ...
        def test7_delete_event_added_in_previous_test(self):
            global token
            self.headers['Authorization'] = f'Rearer {token}'
            ...
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.json()["isDeleted"])
